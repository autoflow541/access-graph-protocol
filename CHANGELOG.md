...
Six real bugs from a pasted external security audit, each verified against actual source before being accepted as real, then fixed with regression tests.

The audit table itself was pasted content, not something to act on directly: per this project's standing rule (see prior audit entries below), every finding was traced through the actual current code before any fix was written. All six held up.

- **Dev services bound to every network interface, not just localhost.** All six `service/run-local*.mjs` entrypoints called `server.listen(port)` with no host argument, which binds `::`/`0.0.0.0` by default: anyone else on the same network could reach a running dev instance. Fixed by passing `"127.0.0.1"` explicitly, matching `service/virtual-thermostat.mjs`'s existing (and already-correct) default. Verified live: booted `run-local.mjs` and confirmed via `netstat` that it listens on `127.0.0.1:8787` only, not a wildcard address.
- **Well-known default dev token.** The same six scripts fell back to the literal string `"dev-token"` whenever `AGP_DEV_TOKEN` was unset, a default that works against any instance of the script, on any machine, indefinitely. New `service/dev-token.mjs` generates a random token per process run instead (still overridable via `AGP_DEV_TOKEN`), computed once and reused for both the caller allowlist and the printed `curl` example, so the two can't drift. Verified live: the same boot printed a random UUID, not `dev-token`.
- **`ExecutionService.confirm()` and `SpecsActionSession.confirm()` accepted any truthy value, not just `true`.** `if (!accepted)` meant `accepted: "false"` (the string), `1`, or `{}` all counted as consent. Both now require `accepted === true`, matching `provideAuthorization()`'s already-strict `result !== true` check in the same file, which is how the inconsistency was spotted (one sibling method already had it right).
- **Idempotency cache checked before authentication, and shared across callers.** `execute()` looked up `this._requestLog.get(requestId)` before calling `_requireProposal()` (where auth actually happens), and the cache key was `requestId` alone. An unauthenticated caller who guessed or observed a `requestId` could get back another caller's cached outcome; two legitimate callers who happened to pick the same `requestId` string could collide. Fixed by authenticating first and keying the cache `callerToken:requestId`.
- **`AccessGraph.register()` silently overwrote a duplicate object id, and never validated `risk`.** A second `register()` call with an id already in the graph replaced the first object with no warning. An action with a risk value outside `RISK_ORDER` (a typo, or a value from an unreleased AGP version) produced `RISK_ORDER.indexOf(risk) === -1`, which the confirmation-gating comparison (`>= indexOf("high")`) evaluated as "less risky than everything," the opposite of what an unrecognized value should mean. `register()` now rejects a duplicate object id, a duplicate action id within one object, and an unrecognized risk value, all at write time. `requiresConfirmationFor()` also now treats an unrecognized risk as requiring confirmation, as a fail-conservative backstop for any action object that reaches it some other way (not just what `register()` lets in).
- **`AccessGraph.get()` returned the live internal object, not a clone.** `list()` and `resolveAction()` already cloned their return values; `get()` didn't, so any caller holding a `get()` result could mutate the real policy graph (add actions, change risk, relabel objects) without going through `register()` or `updateState()` at all. `get()` now clones, consistent with the other two read paths. The internal `require()` accessor, which `updateState()` genuinely needs to mutate, is unchanged.
- **HTTP request bodies had no size limit.** `readJsonBody()` in `service/http-server.js` buffered every chunk of a POST body via `Buffer.concat`, unbounded, before any authentication ran: an unauthenticated memory-exhaustion vector. Added a `maxBodyBytes` constructor option (default 25MB, sized for a base64-encoded PDF or photo) to `createExecutionHttpServer()`; exceeding it returns `413 PAYLOAD_TOO_LARGE` and stops buffering the remainder without tearing down the socket mid-response (an earlier version of this fix called `req.destroy()` immediately, which killed the connection before a proper 413 could be written; the test that caught this crashed the whole Node process, not just the request).

New `tests/security-fixes-test.mjs` reproduces each of the six exploit shapes against the un-fixed behavior's premise before asserting the fix; `tests/execution-http-server-test.mjs` gained a real-socket oversized-body case. All 17 suites and `npm run check:conformance` pass. `SECURITY.md` documents all six as fixed, with the same detail as this entry.
